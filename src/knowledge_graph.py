...
def load_cached_graph(file_name):
    """Load graph data from JSON cache."""
    cache_file_name = f"data/{file_name}_cache.json"
    if os.path.exists(cache_file_name):
        with open(cache_file_name, "r") as f:
            return json.load(f)
    return None

def save_graph_to_cache(nodes, relationships, file_name):
...
def extract_graph_from_text(text, file_name):
    """Extract graph from text with caching."""
    cached_graph = load_cached_graph(file_name)
    if cached_graph:
        ...
        # Save to cache
        save_graph_to_cache(nodes, relationships, file_name)
...
if __name__ == "__main__":
    input_data_dir = 'course_notes'
    pdf_file_name = 'graph-rag-ex-cn.pdf'
    file_path = f'{input_data_dir}/{pdf_file_name}'
    
    pdf_text = read_pdf_file(file_path)

    nodes, relationships = extract_graph_from_text(pdf_text, pdf_file_name)
